#from calculadoraexepciones import MostrarMenu
...
class Agenda:
    ...
    def CargarContactos(self):
        try:
            ...
            print("Error: No existe fichero en la agenda")
        else:
            ...
            if(len(contactos) > 0):
                ...
                print("INFO: se ha cargado un total de ", len(self.__ListaContactos)," contactos")
    ...
    def BorrarContactoPorNombre(self, nombre):
        listafinal = []
        for contacto in self.__ListaContactos:
            if contacto in self.__ListaContactos:
                if contacto.GetNombre() != nombre:
    ...
    3)Volver""")
        ...
        while not finbuscar:
            ...
            if opcbuscar == 1:
                ...
                encontrados = agenda.BorrarContactoPorTelefono(input((">Introduce el telefono a buscar: ")))
                if len(encontrados) > 0:
                    print("######### CONTACTOS ENCONTRADOS ########")
                    for item in encontrados:
                        item.MostrarContacto()
...
def BorrarContacto(agenda):
        print("""Nuscar contactos a borrar por:
        1)Nombre
        2)Telefono
        3)Volver""")
